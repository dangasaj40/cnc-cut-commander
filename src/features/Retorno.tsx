...
export default function RetornoPage() {
  ...
  const fetchInitialData = async () => {
    // Buscar emissões que possuem nestings não finalizados
    ...
    // Buscar operadores e motivos de parada dos ajustes do sistema
  ...
  const handleFinish = async (nesting: string) => {
    if (!form.operador || !form.horaInicio || !form.horaFim || !form.carreiraChapa) {
      alert("Preencha todos os campos obrigatórios (Operador, Horários e Carreira Chapa).");
      return;
    }
    ...
    try {
      ...
      const { error: logError } = await supabase.from("log_retorno").insert({
        ...
        houve_parada: form.houveParada,
        motivo_parada: form.motivoParada,
        obs: form.obs,
      ...
      // 3. Atualizar status na tabela EMISSOES
      ...
      alert(`Baixa do Nesting ${nesting} concluída!`);
      fetchEmissionData();
      fetchInitialData();
    } catch (e: any) {
                   ...
                   {nestings.map(n => (
                     ...
                     >
                        <div className="md:col-span-3">
                           <div className="text-[9px] uppercase font-bold text-muted-foreground mb-1">Nesting</div>
                           <div className="text-lg font-black text-white">{n.nesting}</div>
                           <div className="text-[10px] text-primary font-bold uppercase">{n.bloco} | {n.painel}</div>
                        </div>
                        ...
                        <div className="md:col-span-5 grid grid-cols-2 gap-4">
                           ...
                           <div className="flex flex-col justify-end gap-3">
                              <label className="flex flex-col gap-1">
                                 ...
                                 <select 
                                   className="field py-1 h-8 text-xs"
                                   value={form.houveParada}
                                   onChange={e => setForm(p => ({ ...p, houveParada: e.target.value }))}
                                 >
                              ...
                              >
                                 {isFinishing ? "..." : <><Save size={12} /> Finalizar</>}
                              </button>
                           </div>
                        </div>

                        {form.houveParada === "Sim" && (
                          <div className="col-span-full pt-4 mt-4 border-t border-white/5 grid grid-cols-1 md:grid-cols-2 gap-4">
                             <label className="flex flex-col gap-1.5">
                                <span className="text-[9px] uppercase font-bold text-muted-foreground">Motivo da Parada</span>
                                <select 
                                  className="field py-1 text-xs"
                                  value={form.motivoParada}
                                  onChange={e => setForm(p => ({ ...p, motivoParada: e.target.value }))}
                                >
                             ...
                             <label className="flex flex-col gap-1.5">
                                ...
                                <input 
                                  type="text" 
                                  className="field py-1 text-xs" 
                                  placeholder="Detalhes sobre a parada ou corte..."
                                  value={form.obs}
